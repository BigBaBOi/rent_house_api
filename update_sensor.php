...
try {
    ...
    $stmt = $conn->prepare("UPDATE Sensors SET sensor_name = ?, sensor_type = ?, value_text = ?, status = ?, battery = ? WHERE sensor_id = ?");
    $stmt->execute([$sensorName, $sensorType, $valueText, $status, $battery, $sensorId]);

    echo json_encode(["status" => "success", "message" => "Sensor updated successfully"]);
